...
function useSounds() {
    ...
    (0, react_1.useEffect)(() => {
        ...
        async function loadAll() {
            ...
            const names = Object.keys(SOUND_MODULES);
            for (const name of names) {
                if (cancelled)
                    break;
                try {
                    const { sound } = await expo_av_1.Audio.Sound.createAsync(SOUND_MODULES[name], {
                        shouldPlay: false,
    ...
    const playSound = (0, react_1.useCallback)((name) => {
    ...
    const toggleMute = (0, react_1.useCallback)(() => {
        setIsMuted((prev) => {
            ...
            async_storage_1.default.setItem(MUTE_STORAGE_KEY, String(next)).catch(() => { });
            return next;
        });
    }, []);
