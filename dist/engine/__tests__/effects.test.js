...
describe('applyPowerCardEffect — Ace', () => {
    it('sets activeSuit to declared suit', () => {
        ...
        const result = (0, effects_1.applyPowerCardEffect)({ rank: 'A', suit: 'hearts' }, 'spades', state);
    ...
    it('allows Ace to declare the same suit already in play', () => {
        ...
        const result = (0, effects_1.applyPowerCardEffect)({ rank: 'A', suit: 'clubs' }, 'hearts', state);
...
describe('applyPowerCardEffect — 2', () => {
    it('adds 2 to pendingPickup and sets type to "2"', () => {
        const state = makeState();
        const result = (0, effects_1.applyPowerCardEffect)({ rank: '2', suit: 'clubs' }, null, state);
    ...
    it('stacks on existing pendingPickup', () => {
        ...
        const result = (0, effects_1.applyPowerCardEffect)({ rank: '2', suit: 'spades' }, null, state);
...
describe('applyPowerCardEffect — 8', () => {
    it('increments skipsRemaining by 1', () => {
        const state = makeState();
        const result = (0, effects_1.applyPowerCardEffect)({ rank: '8', suit: 'hearts' }, null, state);
    ...
    it('stacks: two 8 effects = skipsRemaining of 2', () => {
        let state = makeState();
        state = (0, effects_1.applyPowerCardEffect)({ rank: '8', suit: 'hearts' }, null, state);
        state = (0, effects_1.applyPowerCardEffect)({ rank: '8', suit: 'clubs' }, null, state);
...
describe('applyPowerCardEffect — black Jack', () => {
    it('adds 7 to pendingPickup and sets type to "jack"', () => {
        const state = makeState();
        const result = (0, effects_1.applyPowerCardEffect)({ rank: 'J', suit: 'spades' }, null, state);
    ...
    it('stacks two black Jacks to 14', () => {
        let state = makeState();
        state = (0, effects_1.applyPowerCardEffect)({ rank: 'J', suit: 'spades' }, null, state);
        state = (0, effects_1.applyPowerCardEffect)({ rank: 'J', suit: 'clubs' }, null, state);
...
describe('applyPowerCardEffect — red Jack (counter)', () => {
    it('resets pendingPickup and pendingPickupType when countering', () => {
        ...
        const result = (0, effects_1.applyPowerCardEffect)({ rank: 'J', suit: 'hearts' }, null, state);
...
describe('applyPowerCardEffect — Queen', () => {
    it('sets phase to "cover"', () => {
        const state = makeState();
        const result = (0, effects_1.applyPowerCardEffect)({ rank: 'Q', suit: 'hearts' }, null, state);
...
describe('applyPowerCardEffect — King', () => {
    it('reverses direction clockwise → anticlockwise', () => {
        ...
        const result = (0, effects_1.applyPowerCardEffect)({ rank: 'K', suit: 'hearts' }, null, state);
    ...
    it('King in 2-player still reverses direction (not a skip)', () => {
        const state = makeState({ direction: 'clockwise' });
        const result = (0, effects_1.applyPowerCardEffect)({ rank: 'K', suit: 'spades' }, null, state);
        expect(result.direction).toBe('anticlockwise');
...
function makePlayState(hand, topDiscard, overrides = {}) {
    const player = { id: 'p1', hand, isHuman: true };
    return {
        deck: Array.from({ length: 20 }, () => ({ rank: '3', suit: 'clubs' })),
        discard: [topDiscard],
...
describe('resolveConflictingEffects — via applyPlay', () => {
    it('[2♥, A♥]: Ace overrides 2 — pendingPickup is 0, activeSuit is set', () => {
        const hand = [{ rank: '2', suit: 'hearts' }, { rank: 'A', suit: 'hearts' }];
        const state = makePlayState(hand, { rank: '2', suit: 'hearts' });
        const result = (0, state_1.applyPlay)(hand, 'spades', state);
        expect(result.pendingPickup).toBe(0);
        expect(result.pendingPickupType).toBeNull();
        expect(result.activeSuit).toBe('spades');
    });
    it('[A♥, 2♥]: 2 overrides Ace — pendingPickup is 2, activeSuit is null', () => {
        const hand = [{ rank: 'A', suit: 'hearts' }, { rank: '2', suit: 'hearts' }];
        const state = makePlayState(hand, { rank: 'A', suit: 'hearts' });
        const result = (0, state_1.applyPlay)(hand, 'hearts', state);
        expect(result.pendingPickup).toBe(2);
        expect(result.pendingPickupType).toBe('2');
        expect(result.activeSuit).toBeNull();
    });
    it('[2♥, 2♦]: same type stacks — pendingPickup is 4', () => {
        const hand = [{ rank: '2', suit: 'hearts' }, { rank: '2', suit: 'diamonds' }];
        const state = makePlayState(hand, { rank: '2', suit: 'hearts' });
        const result = (0, state_1.applyPlay)(hand, null, state);
        expect(result.pendingPickup).toBe(4);
        expect(result.pendingPickupType).toBe('2');
    });
    ...
    it('[K♠, A♠]: A last — K direction preserved, A activeSuit preserved, no conflict', () => {
        ...
        const cards = [{ rank: 'K', suit: 'spades' }, { rank: 'A', suit: 'spades' }];
        const result = (0, state_1.resolveConflictingEffects)(state, cards);
        expect(result.direction).toBe('anticlockwise');
        expect(result.activeSuit).toBe('hearts');
    });
    ...
    it('[A♥, K♥]: K last — clears activeSuit set by A', () => {
        ...
        const cards = [{ rank: 'A', suit: 'hearts' }, { rank: 'K', suit: 'hearts' }];
        const result = (0, state_1.resolveConflictingEffects)(state, cards);
        expect(result.activeSuit).toBeNull();
        expect(result.direction).toBe('anticlockwise');
    });
    ...
    it('[J♠, A♠]: A last — clears pendingPickup from black Jack, activeSuit preserved', () => {
        ...
        const cards = [{ rank: 'J', suit: 'spades' }, { rank: 'A', suit: 'spades' }];
        const result = (0, state_1.resolveConflictingEffects)(state, cards);
        expect(result.pendingPickup).toBe(0);
        expect(result.pendingPickupType).toBeNull();
    ...
    it('[2♥, 8♥]: 8 last — clears pendingPickup from 2, skipsRemaining preserved', () => {
        ...
        const cards = [{ rank: '2', suit: 'hearts' }, { rank: '8', suit: 'hearts' }];
        const result = (0, state_1.resolveConflictingEffects)(state, cards);
        expect(result.pendingPickup).toBe(0);
        expect(result.pendingPickupType).toBeNull();
        expect(result.skipsRemaining).toBe(1);
