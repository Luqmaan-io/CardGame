...
exports.canWinNextTurn = canWinNextTurn;
...
function getEffectiveSuit(state) {
    ...
    if (state.activeSuit !== null)
        return state.activeSuit;
    return top ? top.suit : null;
}
function isValidPlay(card, state) {
    const top = getTopDiscard(state);
    if (!top)
        return true; // empty discard — anything goes
    ...
    if (pendingPickupType === 'jack') {
        return (0, types_1.isBlackJack)(card) || (0, types_1.isRedJack)(card);
    }
    ...
    if (card.rank === 'Q') {
        ...
        if (!currentPlayer)
            return false;
        const hasCover = currentPlayer.hand.some((c) => c !== card && (c.rank === 'Q' || c.suit === card.suit));
        if (!hasCover)
            return false;
    }
    ...
    if (effectiveSuit !== null && card.suit === effectiveSuit)
        return true;
 ...
 */
function canWinNextTurn(playerId, state) {
    const player = state.players.find((p) => p.id === playerId);
    if (!player || player.hand.length === 0)
        return false;
    return _hasWinningCombo(player.hand);
}
function _hasWinningCombo(hand) {
    for (let startIdx = 0; startIdx < hand.length; startIdx++) {
        const remaining = [...hand];
        const first = remaining.splice(startIdx, 1)[0];
        if (_findWinningSequence([first], remaining, null))
            return true;
    }
    return false;
}
function _findWinningSequence(current, remaining, direction) {
    ...
    for (let i = 0; i < remaining.length; i++) {
        ...
        if (!_canFollow(lastCard, nextCard, pendingQueenSuit, direction))
            continue;
        ...
        if (_findWinningSequence([...current, nextCard], newRemaining, newDirection))
            return true;
    }
    return false;
}
function _canFollow(prev, next, pendingQueenSuit, currentDirection) {
    ...
    if (prev.suit === next.suit) {
        ...
        if (currentDirection !== null && moveDir !== currentDirection)
            return false;
        return true;
    }
    return false;
}
function _getPendingQueenSuit(combo) {
...
function _validateQueenCovers(combo) {
    for (let i = 0; i < combo.length; i++) {
        ...
        if (next.suit !== combo[i].suit && next.rank !== 'Q')
            return false;
    }
    return true;
}
function isValidCombo(cards, state) {
    ...
    for (let i = 1; i < cards.length; i++) {
        ...
        if (needsCoverSuit !== null) {
            if (curr.rank !== 'Q' && curr.suit !== needsCoverSuit)
                return false;
            needsCoverSuit = null;
