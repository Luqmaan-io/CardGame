import { drawCard, advanceTurn } from '../state';
...
function makeCards(n: number, suit: Card['suit'] = 'clubs'): Card[] {
...
describe('drawCard — 14 cards, sufficient deck', () => {
  it('hand increases by exactly 14', () => {
    ...
    const result = drawCard(14, state);
...
describe('drawCard + advanceTurn — penalty draw flow', () => {
  it('turn advances to next player after penalty draw', () => {
    ...
    const drawn = drawCard(14, state);
    const advanced = advanceTurn(drawn);
